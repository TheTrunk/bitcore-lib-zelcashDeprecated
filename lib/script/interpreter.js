...
Interpreter.prototype.CheckBlockHash = function(nHeight, vchCompareTo) {
...
Interpreter.prototype.step = function() {
  ...
  if (fExec && 0 <= opcodenum && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
      ...
      case Opcode.OP_NOP5:
      case Opcode.OP_CHECKBLOCKATHEIGHT:
        {
          ...
          var vchBlockIndex = this.stack[this.stack.length - 1]
          var vchBlockHash = this.stack[this.stack.length - 2]

          var nHeight = BN.fromScriptNumBuffer(vchBlockIndex, true, 4).toNumber;

          if (nHeight < 0 || !this.CheckBlockHash(nHeight, vchBlockHash)) {
            this.errstr = 'SCRIPT_ERR_INVALID_STACK_OPERATION';
            return false;
          }
